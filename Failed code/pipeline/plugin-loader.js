...
class PluginLoader {
  ...
  async healthCheck() {
    try {
      ...
      const pluginHealthChecks = await Promise.allSettled(
        enabledPlugins.map(async (pluginId) => {
          const plugin = this.registeredPlugins.get(pluginId);
      ...
      const healthyPlugins = pluginHealthChecks.filter(
        result => result.status === 'fulfilled' && result.value.health.status === 'healthy'
      ).length;
