...
export const analyzeImageRoute = async (req, res) => {
    ...
    try {
        ...
        const response = await enhancedAIProcessor.processAnalysisRequest(imageData || defaultImageData, // Pass valid empty base64 for text-only requests
        message, userContext.userId, options); // Type assertion to handle Enhanced AI Processor response
