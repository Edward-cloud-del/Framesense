...
router.post('/create-checkout-session', authenticateUser, async (req, res) => {
    try {
        ...
        const paymentToken = jwt.sign({ userId: user.id, email: user.email, plan: planName }, JWT_SECRET, { expiresIn: '1h' });
        ...
        const session = await subscriptionService.createCheckoutSession(customerId, priceId, successUrl, cancelUrl, user.id, // Pass user ID for webhook
...
router.get('/user-tier-info', authenticateUser, async (req, res) => {
    try {
        const user = req.user;
