...
router.post('/analyze', authenticateUser, upload.single('image'), async (req, res) => {
    ...
    try {
        const { question, modelPreference, forceModel = false, cacheStrategy = 'default' } = req.body;
        let imageData;
        ...
        const result = await enhancedAIProcessor.processAnalysisRequest(imageData, question, req.user.id, options);
...
router.put('/user/model-preferences', authenticateUser, async (req, res) => {
    try {
        const { defaultModel, autoUpgrade, budgetLimit, costOptimization } = req.body;
...
router.get('/usage/stats', authenticateUser, async (req, res) => {
    try {
        const { period = '7d', breakdown = 'service', detailed = 'false' } = req.query;
