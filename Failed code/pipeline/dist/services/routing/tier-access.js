...
class TierAccess {
    ...
    async validateAccess(questionType, userProfile) {
        ...
        console.log(`🛡️ === ENHANCED TIER ACCESS VALIDATION ===`);
        console.log(`Question Type ID: ${questionType.id}`);
        console.log(`Question Type Object:`, JSON.stringify(questionType, null, 2));
        ...
        try {
            ...
            console.log(`📋 TIER PERMISSIONS FOR '${userProfile.tier}':`, JSON.stringify(tierPermissions, null, 2));
            if (!tierPermissions) {
                console.error(`❌ INVALID TIER: '${userProfile.tier}' not found in TIER_PERMISSIONS`);
                console.error(`Available tiers:`, Object.keys(this.TIER_PERMISSIONS));
            ...
            console.log(`🔍 QUESTION TYPE TIER MAPPING:`);
            console.log(`  Question Type: ${questionType.id}`);
            console.log(`  Required Tier: ${requiredTier || 'NOT DEFINED'}`);
            console.log(`  User Tier: ${userProfile.tier}`);
            ...
            if (userProfile.tier === 'pro' && tierPermissions.questionTypes.includes('all')) {
                ...
                if (!canAccess && requiredTier) {
                    ...
                    return this.createAccessDeniedResult('tier_insufficient', requiredTier, `${questionType.id} requires ${requiredTier} tier or higher`);
                }
            }
            // Check daily usage limit
            const dailyUsage = await this.getDailyUsage(userProfile.userId);
            if (dailyUsage >= tierPermissions.dailyLimit) {
                return this.createAccessDeniedResult('daily_limit_exceeded', userProfile.tier === 'free' ? 'pro' : 'premium', `Daily limit of ${tierPermissions.dailyLimit} requests exceeded`);
            }
            // Check monthly usage limit
            const monthlyUsage = await this.getMonthlyUsage(userProfile.userId);
            if (monthlyUsage >= tierPermissions.monthlyLimit) {
                return this.createAccessDeniedResult('monthly_limit_exceeded', userProfile.tier === 'free' ? 'pro' : 'premium', `Monthly limit of ${tierPermissions.monthlyLimit} requests exceeded`);
            }
            ...
            if (activeRequests > tierPermissions.maxConcurrentRequests) {
                return this.createAccessDeniedResult('concurrent_limit_exceeded', userProfile.tier, `Maximum ${tierPermissions.maxConcurrentRequests} concurrent requests allowed`);
            }
            ...
            if (tierPermissions.costLimit) {
                ...
                if (monthlyCost + estimatedCost > tierPermissions.costLimit) {
                    return this.createAccessDeniedResult('cost_limit_exceeded', 'premium', `Monthly cost limit of $${tierPermissions.costLimit} would be exceeded`);
                }
            }
            // All checks passed
    ...
    getTierUpgradeRecommendations(userProfile, deniedQuestionTypes = []) {
        ...
        return recommendations.filter(rec => deniedQuestionTypes.some(qt => rec.enabledQuestionTypes.includes(qt)));
    }
    /**
