...
class SmartRouter {
    ...
    async prepareFinalRoute(route, questionType, userProfile) {
        ...
        // Add fallback chain
        route.fallbackChain = await this.fallbackManager.buildFallbackChain(route.service, questionType, userProfile.tier);
        // Add routing flags
        route.flags = {
            requiresAuth: true,
            trackUsage: true,
    ...
    findBestModelForTier(questionType, availableModels, userProfile) {
        ...
        const scoredModels = compatibleModels.map(model => {
            ...
            const capabilityMatch = questionCapabilities.filter(cap => model.capabilities.includes(cap)).length;
