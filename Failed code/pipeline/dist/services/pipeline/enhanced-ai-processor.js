...
class EnhancedAIProcessor {
    constructor() {
        ...
        this.smartRouter = new SmartRouter(this.questionClassifier, this.modelSelector, this.tierAccess, this.costOptimizer, this.fallbackManager);
        // Service instances
    ...
    async processAnalysisRequest(imageData, question, userId, options = {}) {
        ...
        try {
            ...
            const cacheKey = await this.cacheManager.generateKey(validatedData.imageData, question, questionType.id, userProfile.tier);
            ...
            const routing = await this.smartRouter.routeRequest(questionType, options.modelPreference, userProfile, options);
...
Please answer the user's question based on the extracted text. Be helpful and specific.`;
    ...
    analyzeTextContext(text) {
        if (!text || text.length < 3)
            return 'very short text or symbols';
        if (/^\d+/.test(text))
            return 'numerical data';
        if (/%|command|terminal|shell/.test(text.toLowerCase()))
            return 'terminal or command output';
        if (/error|warning|fail/.test(text.toLowerCase()))
            return 'error or warning message';
        if (/welcome|hello|greet/.test(text.toLowerCase()))
            return 'greeting or welcome text';
        return 'text content';
    }
    /**
    ...
    isTextOnlyRequest(questionType, question) {
        ...
        if (questionType.id === 'PURE_TEXT') {
            ...
            const hasImageContentKeywords = imageContentKeywords.some(keyword => normalizedQuestion.includes(keyword));
    ...
    async healthCheck() {
        try {
            ...
            const allHealthy = Object.values(componentHealth).every(status => status === 'healthy' || (typeof status === 'object' && status.status === 'healthy'));
            return {
