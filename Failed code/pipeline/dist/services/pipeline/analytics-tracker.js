...
class AnalyticsTracker {
    ...
    async trackRequest(requestData) {
        ...
        try {
            const { userId, userTier = 'free', questionType, service, model, responseTime, success = true, cached = false, confidence = null, error = null, imageSize = 0, questionText = '' } = requestData;
    ...
    calculateRequestCost(service, userTier, cached = false) {
        if (cached)
            return 0; // No cost for cached requests
    ...
    async saveAnalyticsData() {
        try {
            ...
            await fs.writeFile(this.persistenceConfig.dataFile, JSON.stringify(dataToSave, null, 2));
        }
        catch (error) {
