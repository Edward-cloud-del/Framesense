...
class OCRService {
    static async extractText(imageBuffer, options = {}) {
        const { language = 'eng', confidence_threshold = 0.5, preprocess = true } = options;
    ...
    static async analyzeImageForText(imageBuffer) {
        ...
        try {
            ...
            const hasContrastIndicatingText = stats.channels.some(channel => channel.std > 30 // Standard deviation indicates contrast
            );
