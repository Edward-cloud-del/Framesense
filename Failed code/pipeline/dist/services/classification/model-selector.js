...
class ModelSelector {
    ...
    getAvailableModels(userTier, requiredCapabilities = []) {
        ...
        return Object.values(this.modelRegistry).filter(model => {
            ...
            if (!model.enabled)
                return false;
            // Check tier access
            if (!this.validateTierAccess(model, userTier))
                return false;
            // Check cost limits
            if (model.costPerRequest > tierInfo.maxCostPerRequest)
                return false;
            // Check capabilities if specified
            if (requiredCapabilities.length > 0) {
                const hasCapabilities = requiredCapabilities.every(cap => model.capabilities.includes(cap));
                if (!hasCapabilities)
                    return false;
            }
            return true;
    ...
    selectModel(questionType, userTier, userPreference = null, options = {}) {
        ...
        const defaultModel = availableModels.find(model => model.id === questionType.defaultModel ||
    ...
    validateTierAccess(model, userTier) {
        ...
        // Check tier level
        if (tierInfo.level < modelTierInfo.level)
            return false;
        // Check provider access
        if (tierInfo.allowedProviders.includes('all'))
            return true;
    ...
    getModelsByCapability(questionType) {
        ...
        return Object.values(this.modelRegistry)
            .filter(model => model.enabled)
            .map(model => {
            const matchingCaps = model.capabilities.filter(cap => requiredCaps.includes(cap));
            return {
                ...model,
