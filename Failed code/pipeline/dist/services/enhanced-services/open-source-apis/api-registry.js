...
class APIRegistry {
    ...
    findPluginsByCapabilities(requiredCapabilities) {
        ...
        for (const [pluginId, instance] of this.pluginInstances) {
            ...
            const hasAllCapabilities = requiredCapabilities.every(cap => pluginCapabilities.includes(cap));
    ...
    async cleanup() {
        ...
        for (const [pluginId, instance] of this.pluginInstances) {
            cleanupPromises.push(instance.cleanup().catch(error => console.error(`Error cleaning up ${pluginId}:`, error)));
        }
    ...
    loadConfig(config) {
        ...
        for (const [pluginId, pluginConfig] of Object.entries(config.plugins || {})) {
            if (pluginConfig.enabled && this.registeredPlugins.has(pluginId)) {
                this.enablePlugin(pluginId).catch(error => console.error(`Failed to auto-enable ${pluginId}:`, error));
            }
        }
    }
}
