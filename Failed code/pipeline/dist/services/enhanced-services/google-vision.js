...
class GoogleVisionService {
    ...
    async detectCelebritiesAndWeb(imageData, options = {}) {
        ...
        try {
            ...
            const celebrities = webEntities.filter(entity => this.isCelebrityEntity(entity.description) && entity.score > 0.7);
    ...
    async comprehensiveAnalysis(imageData, features = [], userTier = 'free') {
        try {
            ...
            if (enabledFeatures.includes('text')) {
                promises.push(this.detectText(imageData).then(result => ({ type: 'text', result })));
            }
            if (enabledFeatures.includes('objects')) {
                promises.push(this.detectObjects(imageData).then(result => ({ type: 'objects', result })));
            }
            if (enabledFeatures.includes('logos')) {
                promises.push(this.detectLogos(imageData).then(result => ({ type: 'logos', result })));
            }
            if (enabledFeatures.includes('celebrities') && userTier === 'premium') {
                promises.push(this.detectCelebritiesAndWeb(imageData).then(result => ({ type: 'celebrities', result })));
            }
